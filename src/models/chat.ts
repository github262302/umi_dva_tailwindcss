...
const ChatModel: ChatModelType = {
    ...
    reducers: {
        ...
        addMsg(state: ChatModelState, { payload }) {
            let next: boolean = state.people.some(e => e.userId == state.selectPeopleId)
            if (!next) {
                let user = state.peopleList.find(e => e.userId == state.selectPeopleId)
